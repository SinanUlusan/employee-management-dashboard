...
import { PersonEditor } from '../components/employee-form.js';
...
suite('employee-form', () => {
  ...
  test('is defined', () => {
    const el = document.createElement('person-editor');
    assert.instanceOf(el, PersonEditor);
  });

  test('renders empty form for new employee', async () => {
    el = await fixture(html`<person-editor></person-editor>`);
    await el.updateComplete;

    assert.exists(el.shadowRoot.querySelector('.form-container'));
    assert.exists(el.shadowRoot.querySelector('form'));
    assert.equal(el.shadowRoot.querySelector('.form-title').textContent.trim(), translate('addNew'));
  ...
  test('renders form with employee data for edit', async () => {
    el = await fixture(html`<person-editor .employee=${mockEmployee}></person-editor>`);
    await el.updateComplete;
  ...
  test('validates required fields', async () => {
    ...
    const fields = ['firstName', 'lastName', 'email', 'phone', 'department', 'position', 'employmentDate', 'birthDate'];
    fields.forEach(field => {
      assert.equal(el.errors[field], translate('required'));
    });
  });
  ...
  test('validates phone format', async () => {
    ...
    assert.equal(el.errors.phone, translate('invalidPhone'));
  });
  ...
  test('validates birth date not in future', async () => {
    ...
    assert.equal(el.errors.birthDate, translate('invalidBirthDate'));
